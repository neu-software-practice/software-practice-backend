...
> 本文档描述 NEUHIS Agent（东软云脑智能医疗）后端服务的整体架构，包括技术栈、依赖关系、分层设计与包结构。与 `SPEC.md`（项目目标与要求）、`rest-api.md`（REST/SSE API 合约）配合阅读。
...
- 实现 `docs/rest-api.md`（及版本化补丁 `rest-api-patch-v*.md`）定义的全部 REST/SSE endpoint
- 作为业务编排层，衔接患者身份、就诊会话生命周期、流程卡交互、支付/取药/治疗执行等医院域逻辑
- 对接 `medAgent` AI 诊疗引擎，将 medAgent 的 `Step` 指令转译为前端可消费的 SSE 事件与时间线条目

**产品名**：东软云脑智能医疗  
**前端合约基线**：`docs/rest-api.md`（由前端 Zod schema 自动梳理，已通过 mock/契约测试），版本化补丁见 `rest-api-patch-v*.md`
...
│   ├── SPEC.md                     # 项目目标与要求
│   ├── STRUCTURE.md                # 本文档
│   └── rest-api.md                 # 前端接口合约（权威 REST/SSE 基线）
...
- Postman Collection 覆盖 `rest-api.md` 核心时序（§7.1–§7.5）
- 所有测试运行 `-race` 标志
