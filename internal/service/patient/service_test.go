...
func TestGetContext_SkipsNonCompletedVisit(t *testing.T) {
	...
	// First visit is in-progress (chatting), second is completed.
	// GetContext must skip the in-progress one and use the completed one.
	visitRepo := &mockVisitRepo{
			...
			return []model.VisitSessionSummary{
				{
					ID:        "v002",
					PatientID: "p001",
					Status:    "chatting", // NOT completed — should be skipped
					Summary:   model.VisitSummary{},
				},
				{
					ID:        "v001",
	...
	if ctx2.PriorVisit.SessionID != "v001" {
		t.Errorf("SessionID = %s, want v001 (completed visit, not v002 which is chatting)", ctx2.PriorVisit.SessionID)
	}
...
func TestGetContext_AllVisitsNonCompleted(t *testing.T) {
	...
	// All visits are non-completed — PriorVisit should be nil.
	visitRepo := &mockVisitRepo{
		listByPatientFunc: func(ctx context.Context, patientID string, cursor *string, pageSize int) ([]model.VisitSessionSummary, *string, bool, error) {
			return []model.VisitSessionSummary{
				{
