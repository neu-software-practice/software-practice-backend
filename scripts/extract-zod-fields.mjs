...
function extractExprConstraints(expr) {
  ...
  if (expr.includes('.regex(')) constraints.hasRegex = true;
  if (expr.includes('.trim()')) constraints.trimmed = true;
  if (expr.includes('.positive()')) constraints.positive = true;
  if (expr.includes('.int()')) constraints.integer = true;
...
/**
 * Map facade method name to input/request schema name.
 * For POST/PATCH/PUT endpoints, this is the request body schema.
 * For GET endpoints, this is the query parameter schema.
 */
function inferRequestSchema(methodName) {
  const inputMap = {
    // Auth
    'login': 'loginInputSchema',
    'register': 'registerFormSchema',
    'refresh': 'refreshInputSchema',
    'logout': null, // no body (204)
    // Patient
    'verifyIdentity': 'verifyIdentityInputSchema',
    'updatePatientProfile': 'updatePatientProfileInputSchema',
    // Visit
    'createSession': 'createSessionInputSchema',
    'createFollowUp': 'createFollowUpInputSchema',
    'generateTitle': 'generateTitleInputSchema',
    // Workbench
    'sendMessage': 'sendMessageInputSchema',
    'submitLabDecision': 'submitLabDecisionInputSchema',
    'submitPayment': 'submitPaymentInputSchema',
    'submitFulfillment': 'submitFulfillmentInputSchema',
    'submitTreatmentExecution': 'submitTreatmentExecutionInputSchema',
    'ackAdvice': 'ackAdviceInputSchema',
    'askLockedQuestion': 'askLockedQuestionInputSchema',
    'classifyFollowUpIntent': 'classifyIntentInputSchema',
    'reportVitals': 'reportVitalsInputSchema',
    'exitVisit': 'exitVisitInputSchema',
    'pauseVisitTimer': 'pauseVisitTimerInputSchema',
    'resumeVisitTimer': 'pauseVisitTimerInputSchema',
    'dismissEmergency': 'dismissEmergencyInputSchema',
    'suspendVisit': 'suspendVisitInputSchema',
    // Address
    'createAddress': 'createAddressInputSchema',
    'updateAddress': 'updateAddressInputSchema',
    'deleteAddress': 'deleteAddressInputSchema',
    'setDefaultAddress': 'setDefaultAddressInputSchema',
    // Admin
    'adminLogin': 'adminLoginInputSchema',
    'updateSettings': 'updateSystemSettingsInputSchema',
    // Query schemas (GET endpoints)
    'listSessions': 'listSessionsInputSchema',
    'getSession': 'listTimelineInputSchema',
    'getReadonlySnapshot': null,
    'getPatientContext': null,
    'listAddresses': null,
    'listBillingRecords': 'listBillingRecordsInputSchema',
    'listMedicalOrders': 'listMedicalOrdersInputSchema',
    'getDashboardStats': null,
    'listPatients_admin': 'adminPatientListQuerySchema',
    'listSessions_admin': 'adminSessionListQuerySchema',
    'getSettings': null,
    'getPatient': null,
  };
  return inputMap[methodName] || null;
}

// ====== Enum Name Mapping: Frontend Zod → Backend Go ======

/**
 * Maps frontend Zod enum schema names to backend Go type names.
 * Used by compare-fields.mjs for enum value comparison.
 */
const ENUM_NAME_MAP = {
  'visitStatusSchema': 'VisitStatus',
  'visitMachineStateSchema': 'VisitMachineState',
  'terminalReasonSchema': 'TerminalReason',
  'paymentStatusSchema': 'PaymentStatus',
  'visitEntryTypeSchema': 'VisitEntryType',
  'flowCardKindSchema': 'FlowCardKind',
  'flowCardStatusSchema': 'FlowCardStatus',
  'timelineItemKindSchema': 'TimelineItemKind',
  'timelineItemStatusSchema': 'TimelineItemStatus',
  'systemEventTypeSchema': 'SystemEventType',
  'sseEventTypeSchema': 'SSEEventType',
  'credentialTypeSchema': 'CredentialType',
  'genderSchema': 'Gender',
  'messageRoleSchema': 'MessageRole',
  'labDecisionSchema': 'LabDecision',
  'paymentPurposeSchema': 'PaymentPurpose',
  'fulfillmentModeSchema': 'FulfillmentMode',
  'treatmentActionSchema': 'TreatmentAction',
  'intentTypeSchema': 'IntentType',
  'emergencySeveritySchema': 'EmergencySeverity',
  'exitReasonSchema': 'ExitReason',
  'exitConsequenceKindSchema': 'ExitConsequenceKind',
  'vitalsSourceSchema': 'VitalsSource',
  'diagnosisConfidenceSchema': 'DiagnosisConfidence',
  'treatmentPlanTypeSchema': 'TreatmentPlanType',
  'capabilitySchema': 'Capability',
  'treatmentExecutionStatusSchema': 'TreatmentExecutionStatus',
  'labExecutionStatusSchema': 'LabExecutionStatus',
  'finalOutcomeSchema': 'FinalOutcome',
  'evidenceSourceSchema': 'EvidenceSource',
  'interruptedBySchema': 'InterruptedBy',
  'medicationFulfillmentStatusSchema': 'MedicationFulfillmentStatus',
  'fulfillmentStatusSchema': 'FulfillmentStatus',
  'timerActionSchema': 'TimerAction',
  'addressTagSchema': 'AddressTag',
};
...
function main() {
  ...
  const enrichedEndpoints = endpointMappings.map(ep => {
    ...
    const requestSchemaName = inferRequestSchema(ep.facadeMethod);
    const reqSchema = requestSchemaName ? allSchemas[requestSchemaName] : null;
    const reqFields = reqSchema ? reqSchema.fields : [];
    const reqFieldMap = {};
    for (const f of reqFields) reqFieldMap[f.name] = f;
  ...
  console.error(`[STATS] ${enrichedEndpoints.filter(e => e.schemaFound).length}/${enrichedEndpoints.length} endpoints have matching response schemas`);
  console.error(`[STATS] ${enrichedEndpoints.filter(e => e.requestSchemaFound).length}/${enrichedEndpoints.length} endpoints have matching request schemas`);
